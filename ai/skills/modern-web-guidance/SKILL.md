...
- Retrieve only guides whose descriptions directly address the task; similarity alone is insufficient. If no result is directly relevant, state that no applicable guide was found and continue. Retrieve multiple relevant IDs in one comma-separated call.
