...
- **Maintainability and conventions:** Assess architecture fit, clarity, duplication, abstraction level, public surface area, and consistency with explicit user preferences and nearby repository patterns. Flag premature generalization. Do not demand generic style churn that does not improve the code.
- **Module boundaries:** Flag code that reaches into another module’s internal implementation, imports unsupported private paths, or monkey-patches exports, prototypes, or runtime behavior instead of using a public contract or changing the owning module.
