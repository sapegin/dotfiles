...
- Every discovered `SKILL.md` is inspected and accounted for in the report.
- Skill-local Markdown references are inspected; external references are followed far enough to verify their pointers and consistency.
- Every skill is checked against both required audit references.
- A comparative pass checks consistency and duplication across the complete inventory, including opportunities for shared references or deterministic scripts.
- Findings cite repository-relative paths and precise line references.
- No skill or instruction file is modified.
...
4. Apply every relevant rubric item to every skill. Verify commands, paths, links, dependencies, and repository conventions from local evidence. Do not infer a defect merely because one skill differs from another.
