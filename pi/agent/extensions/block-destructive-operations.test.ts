...
import blockDestructiveOperations, {
...
function setupExtension() {
  ...
  const select = vi.fn<() => Promise<string>>(() => Promise.resolve('Allow'));
  const pi = {
    events: { emit },
  ...
  blockDestructiveOperations(pi);

  return {
    emit,
    runToolCall(command: string) {
      return toolCallHandler?.({ toolName: 'bash', input: { command } }, ctx);
    },
    select,
  };
}

describe('block destructive operations extension', () => {
  test('signals that input is required before requesting permission', async () => {
    const extension = setupExtension();

    await extension.runToolCall('rm dist/file.txt');

    expect(extension.emit).toHaveBeenCalledExactlyOnceWith(
      permissionRequiredEvent,
      undefined
    );
    expect(extension.select).toHaveBeenCalledOnce();
    expect(extension.emit.mock.invocationCallOrder[0]).toBeLessThan(
      extension.select.mock.invocationCallOrder[0] ?? 0
    );
  });
});
